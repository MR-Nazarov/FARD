...
def convert_to_one_hot(x):
    ...
    one_hot = torch.permute(one_hot[:, 0],
                            (0, -1, *tuple(range(1, x.ndim - 1))))
...
class SymmetricFocalLoss(nn.Module):
    ...
    def forward(self, y_pred, y_true):
        ...
        y_true = convert_to_one_hot(y_true)
        cross_entropy = -y_true * torch.log(y_pred)

        separate_ce = torch.pow(1 - y_pred, self.gamma) * cross_entropy
        separate_ce = torch.cat(((1 - self.delta) * separate_ce[:, :1],
                                 self.delta * separate_ce[:, 1:]), dim=1)
        loss = torch.sum(separate_ce,
                         dim=(0, *tuple(range(2, y_pred.ndim))))
...
class AsymmetricFocalLoss(nn.Module):
    ...
    def forward(self, y_pred, y_true):
        ...
        back_ce = (1 - self.delta) * \
                  (torch.pow(1 - y_pred[:, :1], self.gamma) *
                   cross_entropy[:, :1])
        fore_ce = self.delta * cross_entropy[:, 1:]
        ce = torch.cat((back_ce, fore_ce),
                       dim=1)
        loss = torch.sum(ce,
                         dim=(0, *tuple(range(2, y_pred.ndim))))
        loss = torch.mean(loss)
...
class SymmetricFocalTverskyLoss(nn.Module):
    ...
    def forward(self, y_pred, y_true):
        y_pred = torch.clamp(y_pred, self.epsilon,
                             1. - self.epsilon)
        axis = (0, *tuple(range(2, y_pred.ndim)))
        ...
        # back_dice = (1 - mTI[:, 0]) * torch.pow(1 - mTI[:, 0], -self.gamma)
        # fore_dice = (1 - mTI[:, 1]) * torch.pow(1 - mTI[:, 1], -self.gamma)
        ...
        separate_dice = (1 - mTI) * \
                        (torch.pow(1 - mTI, -self.gamma))
        loss = torch.mean(separate_dice)
        return loss
...
class AsymmetricFocalTverskyLoss(nn.Module):
    ...
    def forward(self, y_pred, y_true):
        ...
        back_mTI = 1 - mTI[:1]
        fore_mTI = torch.sum(torch.pow(1 - mTI[1:], 1 - self.gamma),
                             dim=0,
                             keepdim=True)

        maFT = torch.cat((back_mTI, fore_mTI))
        loss = torch.sum(maFT)
        ...
        # back_dice = (1 - mTI[:, 0])
        # fore_dice = (1 - mTI[:, 1]) * \
        #             (torch.pow(1 - mTI[:, 1], -self.gamma))
        #
        # # Average class scores
        # loss = torch.mean(torch.stack([back_dice,
        #                                fore_dice], dim=-1))
        ###########################################
...
class SymmetricUnifiedFocalLoss(nn.Module):
    ...
    def forward(self, y_pred, y_true):
        symmetric_ftl = SymmetricFocalTverskyLoss(delta=self.delta,
                                                  gamma=self.gamma)(y_pred, y_true)
        symmetric_fl = SymmetricFocalLoss(delta=self.delta,
                                          gamma=self.gamma)(y_pred, y_true)
        if self.weight is not None:
            return (self.weight * symmetric_ftl) + \
                   ((1 - self.weight) * symmetric_fl)
        else:
...
class AsymmetricUnifiedFocalLoss(nn.Module):
    ...
    def forward(self, y_pred, y_true):
        ...
        # Obtain Asymmetric Focal losAsymmetricFocalLosss
